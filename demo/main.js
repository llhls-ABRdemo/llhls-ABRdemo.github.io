...
let switchNums = [0, 0, 0, 0, 0];
let lastLevels = [-1, -1, -1, -1, -1];
let page1Dom;
let page2Dom;
...
$(document).ready(function () {
  ...
  page1Dom = $('#two-comparison-page');
  page2Dom = $('#five-videos-page');
  $('#five-videos-page').remove();
  ...
  $('#streamSelect').change(function () {
    ...
    loadSelectedStream();
    switchNums = [0, 0, 0, 0, 0];
    lastLevels = [-1, -1, -1, -1, -1];
  ...
  $('.rules').change(function (event) {
    ...
    const index = Number(ruleID.slice(-1)) - 1;
    const lastRule = hlsS[index].abrController.ABRRule;
    const rule = $('#' + ruleID).val();
  ...
  $('#dynamicTLatency').click(function () {
    ...
    if (radioButtons !== null)
      if (dynamicTLatencyOn) radioButtons.style.visibility = 'hidden';
      else radioButtons.style.visibility = 'visible';
  });
  ...
  $('#mybutton1').click(function () {
    const value = Number($(this).attr('value'));
    const mybutton1 = document.getElementById('mybutton1');
    if (mybutton1 !== null) {
      if (value === 1) {
        page1Dom.remove();
        $('.pages').append(page2Dom);
        videos[0] = document.querySelector('#video1-2');
        videos[1] = document.querySelector('#video2-2');
        mybutton1.value = '2';
        mybutton1.textContent = '2 ABRs';
        loadSelectedStream();
        switchNums = [0, 0, 0, 0, 0];
        lastLevels = [-1, -1, -1, -1, -1];
        hlsS[0].abrController.ABRRule = 'LoLp';
        hlsS[1].abrController.ABRRule = 'Default';
        let _chart = Chart.getChart('myChart1');
        if (_chart !== undefined) _chart.destroy();
        _chart = Chart.getChart('myChart2');
        if (_chart !== undefined) _chart.destroy();
      } else if (value === 2) {
        page2Dom.remove();
        $('.pages').append(page1Dom);
        videos[0] = document.querySelector('#video1');
        videos[1] = document.querySelector('#video2');
        mybutton1.value = '1';
        mybutton1.textContent = '5 ABRs';
        loadSelectedStream();
        switchNums = [0, 0, 0, 0, 0];
        lastLevels = [-1, -1, -1, -1, -1];
        hlsS[0].abrController.ABRRule = $('#ABRRule1').val();
  ...
  $('input[type=radio][name=TLatency]').change(function () {
    const newLatency = Number(this.value);
    hlsS.forEach(function (hls) {
...
function loadSelectedStream() {
  ...
  videos.forEach(function (video, i) {
    if (i === 0) hlsS.push(new Hls(hlsConfigLoLp));
    else hlsS.push(new Hls(hlsConfig));
  });
...
function InitialChart(i) {
  const chartName = 'myChart' + (i + 1);
  var ctx = document.getElementById(chartName);
  const _chart = Chart.getChart(chartName);
  if (_chart !== undefined) _chart.destroy();
  const labels = [];
  const data = {
    labels: labels,
  ...
  setInterval(function () {
    var data = [];
    const _throughput = hlsS[i].abrController.bwEstimator
      ? hlsS[i].bandwidthEstimate / 1000.0
      : hlsS[i].config.abrEwmaDefaultEstimate / 1000.0;
    const _bitrate = Math.round(
      hlsS[i].levels[hlsS[i].levelController.currentLevelIndex].bitrate / 1000
    );
...
function logSaver() {
  ...
  setInterval(() => {
    hlsS.forEach(function (hls, i) {
      const index = i + 1;
      // update status
      let idThroughput = '#throughput' + index;
      let idBitrate = '#bitrate' + index;
      let idSwitchnum = '#switchnum' + index;
      let idStalldur = '#stallbur' + index;
      let idLatency = '#latency' + index;
      let idBuflen = '#buflen' + index;
      let idPlaybackR = '#playbackR' + index;
      const _throughput = hls.abrController.bwEstimator
        ? hls.bandwidthEstimate / 1000.0
        : hls.config.abrEwmaDefaultEstimate / 1000.0;
      const _bitrate = Math.round(
        hls.levels[hls.levelController.currentLevelIndex].bitrate / 1000
      );
      const _stalldur = hls.streamController.gapController.stallDuration / 1000;
      const _latency = hls.latency;
      const _playbackR = hls.media.playbackRate;
      if (lastLevels[i] !== -1 && lastLevels[i] !== _bitrate) switchNums[i]++;
      lastLevels[i] = _bitrate;
      const _switchnum = switchNums[i];
      const r = videos[i].buffered;
      const pos = videos[i].currentTime;
      let _bufferLen = 0;
