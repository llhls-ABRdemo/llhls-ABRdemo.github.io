...
export class TimelineChart {
  ...
  constructor(canvas: HTMLCanvasElement, chartJsOptions?: any) {
    ...
    canvas.onclick = (event: MouseEvent) => {
      ...
      if (element.length && chart.data.datasets) {
        ...
        console.log(obj);
        self.hlsS.forEach(function (hls) {
          if (hls?.media) {
  ...
  updateLevels(levels: Level[], levelSwitched) {
    ...
    const { loadLevel, nextLoadLevel, nextAutoLevel } = self.hlsS[0];
    // eslint-disable-next-line no-undefined
    const currentLevel =
      levelSwitched !== undefined ? levelSwitched : self.hlsS[0].currentLevel;
  ...
  updateSubtitleTracks(subtitles: MediaPlaylist[]) {
    ...
    const { subtitleTrack } = self.hlsS[0];
  ...
  drawCurrentTime() {
    const chart = this.chart;
    if (self.hlsS && self.hlsS[0]?.media && chart.data.datasets!.length) {
      const currentTime = self.hlsS[0].media.currentTime;
      const scale = this.chartScales[X_AXIS_SECONDS];
      const ctx = chart.ctx;
      ...
      if (x > chartArea.left && x < chartArea.right) {
        ctx.fillStyle = this.getCurrentTimeColor(self.hlsS[0].media);
  ...
  drawLineX(ctx, x: number, chartArea) {
    ...
    if (x > chartArea.left && x < chartArea.right) {
      ctx.lineWidth = 1;
      ctx.strokeStyle = this.getCurrentTimeColor(self.hlsS[0].media); // alpha '0.5'
      ctx.beginPath();
