...
let dynamicTLatencyOn = getDemoConfigPropOrDefault('dynamicTLatency', true);
let boxThroughputOn = getDemoConfigPropOrDefault('boxThroughputOn', true);
...
videos.push(document.querySelector('#video3'));
videos.push(document.querySelector('#video4'));
videos.push(document.querySelector('#video5'));
...
$(document).ready(function () {
  ...
  const videoWidth = videos[0].style.width;
  if (videoWidth) {
  ...
  // document.querySelector('#ABRRule').selectedIndex = 2;
  ...
  $('#streamSelect').change(function () {
    ...
    loadSelectedStream();
    hlsS[0].abrController.ABRRule = 'LoLp';
    hlsS[1].abrController.ABRRule = 'Default';
    hlsS[2].abrController.ABRRule = 'L2ARule';
    hlsS[3].abrController.ABRRule = 'Llama';
    hlsS[4].abrController.ABRRule = 'StallionRule';
    // hlsS.forEach(function(hls) {
    //   hls.abrController.ABRRule = $('#ABRRule').val();
    // })
  });
  ...
  $('#videoSize').change(function () {
    $('#video1').width($('#videoSize').val());
    $('#bufferedCanvas').width($('#videoSize').val());
    checkBuffer();
    resize();
  });

  $('#ABRRule').change(function () {
    const lastRule = hlsS[0].abrController.ABRRule;
    const rule = $('#ABRRule').val();
    if (rule === 'LoLp') {
      const _hlsjsDefaults = {
        debug: true,
        enableWorker: true,
        lowLatencyMode: true,
        progressive: true,
      ...
      const hlsConfig = $.extend(
        {},
        _hlsjsDefaults,
        setEditorValue(_hlsjsDefaults, { parse: true })
      );
    ...
    hlsS.forEach(function (hls) {
      hls.abrController.lastABRRule = lastRule;
      hls.abrController.ABRRule = rule;
    });
  });
...
function loadSelectedStream() {
  ...
  videos.forEach(function () {
    self.hlsS.push(new Hls(hlsConfig));
  });
  ...
  self.hlsS.forEach(function (hls, i) {
    ...
    hls.on(Hls.Events.FRAG_BUFFERED, function (eventName, data) {
      ...
      if (data.frag.type === 'main') {
        ...
        if (
          events.loadedFrag.length !== 0 &&
          events.loadedFrag[events.loadedFrag.length - 1].id2 === eventFrag.id2
        ) {
        ...
        // update status
        let idBitrate = '#bitrate' + i;
        let idSwitchnum = '#switchnum' + i;
        let idStalldur = '#stallbur' + i;
        let idLatency = '#latency' + i;
        let idBuflen = '#buflen' + i;
        let idPlaybackR = '#playbackR' + i;
        const _bitrate = Math.round(hls.levels[event.id].bitrate / 1000);
        const _latency = hls.latency;
...
function handleVideoEvent(evt) {
  const index = Number(evt.type.slice(-1));
...
function checkBuffer() {
  ...
  const r = videos[0].buffered;
  const seekableEnd = getSeekableEnd();
  let bufferingDuration;
  if (r) {
    ctx.fillStyle = 'black';
    if (!canvas.width || canvas.width !== videos[0].clientWidth) {
      canvas.width = videos[0].clientWidth;
    }
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    const pos = videos[0].currentTime;
    let bufferLen = 0;
    ctx.fillStyle = 'gray';
    for (let i = 0; i < r.length; i++) {
    ...
    if ($('#statsDisplayTab').is(':visible')) {
      let log = `Duration: ${
        ...
        videos[0].seekable
      )}\nPlayed: ${timeRangesToString(videos[0].played)}\n`;
      if (hlsS[0].media) {
        ...
        const videoPlaybackQuality = videos[0].getVideoPlaybackQuality;
        if (
          videoPlaybackQuality &&
          typeof videoPlaybackQuality === typeof Function
        ) {
          log += `Dropped frames: ${
            videos[0].getVideoPlaybackQuality().droppedVideoFrames
          }\n`;
          log += `Corrupted frames: ${
            videos[0].getVideoPlaybackQuality().corruptedVideoFrames
          }\n`;
        } else if (videos[0].webkitDroppedFrameCount) {
          log += `Dropped frames: ${videos[0].webkitDroppedFrameCount}\n`;
        }
      }
      log += `Bandwidth Estimate: ${hlsS[0].bandwidthEstimate.toFixed(3)}\n`;
      if (events.isLive) {
        log +=
          'Live Stats:\n' +
          `  Max Latency: ${hlsS[0].maxLatency}\n` +
          `  Target Latency: ${hlsS[0].targetLatency}\n` +
          `  Latency: ${hlsS[0].latency}\n` +
          `  Edge Stall: ${hlsS[0].latencyController.edgeStalled}\n` +
          `  Playback rate: ${videos[0].playbackRate.toFixed(2)}\n`;
      }
    ...
    ctx.fillStyle = 'blue';
    const x = (videos[0].currentTime / seekableEnd) * canvas.width;
...
function onClickBufferedRange(event) {
  ...
  videos.forEach(function (video, i) {
    const target =
      ((event.clientX - canvas.offsetLeft) / canvas.width) * getSeekableEnd(i);
...
function getLevelButtonHtml(key, levels, onclickReplace, autoEnabled, index) {
  ...
  return (
    ...
    levels
      .map((level, i) => {
        const enabled = hlsS[index][key] === i;
...
function updateLevelInfo(index) {
  ...
  const levels = hlsS[index].levels;
  if (!levels) {
    return;
  }
  ...
  const htmlCapLevel = getLevelButtonHtml(
    'autoLevelCapping',
    levels,
    `levelCapping=hlsS[${index}].$1=$2;updateLevelInfo();onDemoConfigChanged();`,
    hlsS[index].autoLevelCapping === -1,
    index
  );
...
function updateAudioTrackInfo(index) {
  ...
  let html1 = '';
  const audioTrackId = hlsS[index].audioTrack;
  const len = hlsS[index].audioTracks.length;
  const track = hlsS[index].audioTracks[audioTrackId];
  ...
  for (let i = 0; i < len; i++) {
    ...
    html1 +=
      `onclick="hlsS[${index}].audioTrack=` +
      i +
      '">' +
      hlsS[index].audioTracks[i].name +
      '</button>';
  }
...
function logSaver() {
  ...
  avgStats[0] =
    ...
    'Throughput_Max (kbps)' +
    ',' +
    'SelectedBitrate_Avg (kbps)' +
    ',' +
    'SelectedBitrate_Std (kbps)' +
    ',' +
    'SelectedBitrate_Min (kbps)' +
    ',' +
    'SelectedBitrate_Max (kbps)' +
    ',' +
    'Latency_Avg (s)' +
    ',' +
    'Latency_Std (s)' +
  ...
  let throughputArr = [];
  let bitrateArr = [];
  let stallDurationArr = [];
  let playbackRateArr = [];
  let QoEArr = [];
  let latencyArr = [];
  let bufferArr = [];

  let SavePlayerStatsIndex = 1;
  let sn = -1;
  const startTime = new Date().getTime();
  let unfinished = true;
  setInterval(function () {
    //let SegmentDuration = ;
    let UTCtime = moment().format(); //Date.now();
    let newSn = events.loadedFrag[events.loadedFrag.length - 1].id2;
    if (events.loadedFrag.length > 1 && sn != newSn) {
      if (events.loadedFrag.length == 2) {
        startDelay = events.loadedFrag[events.loadedFrag.length - 2].delay;
        videoStartT = events.loadedFrag[events.loadedFrag.length - 2].fragStart;
      } else if (events.loadedFrag.length > 2) {
        videoLen =
          events.loadedFrag[events.loadedFrag.length - 2].fragStart +
          events.loadedFrag[events.loadedFrag.length - 2].dur -
          videoStartT;
      }
      console.log(`====sn:${sn}==========newSn:${newSn}====`);
      sn = newSn;
      var currentLatency = hls.latency;
      var availibleBW = hls.abrController.bwEstimator
        ? hls.bandwidthEstimate / 1000.0
        : hls.config.abrEwmaDefaultEstimate / 1000.0;
      var bitrate = events.level[events.level.length - 1].bitrate;
      const currentBuffer = events.buffer[events.buffer.length - 1].buffer;
      const stallDuration = hls.streamController.gapController.stallDuration;
      hls.streamController.gapController.stallDuration = 0;
      if (stallDuration > 0) {
        stallDurationArr.push(stallDuration / 1000);
        console.log(`>>> stall: ${stallDuration} >>>`);
        lastStall = stallDuration;
        stallNumber++;
      }
      // const stallDuration = hls.latencyController.edgeStalled;
      const lolpQoE = hls.abrController.lolpQoE;
      const playbackRate = hls.media.playbackRate;
      const downloadTimeInMilliseconds =
        events.loadedFrag[events.loadedFrag.length - 2].download;
      const downloadBytes =
        events.loadedFrag[events.loadedFrag.length - 2].fragSize;
      const switchNumber = events.level.length;
      if (lastBitrate && bitrate !== lastBitrate) _switchNumber++;
      lastBitrate = bitrate;
      if (
        availibleBW !== 500 &&
        currentLatency < 30 &&
        ...
        latencyArr.push(currentLatency);
        throughputArr.push(availibleBW);
        bitrateArr.push(bitrate);
        bufferArr.push(currentBuffer / 1000);
        QoEArr.push(lolpQoE);
        playbackRateArr.push(playbackRate);
        const segmentObj = {
          bitrate: bitrate,
          codec: 'h264',
          duration: events.loadedFrag[events.loadedFrag.length - 2].dur,
          fps: hls.currentFPS,
          resolution: `${video.videoWidth}x${video.videoHeight}`,
          start: events.loadedFrag[events.loadedFrag.length - 2].fragStart,
        };
        jsonObj.I13.segments.push(segmentObj);
        if (stallDuration > 0) {
          const stallObj = [
            hls.streamController.gapController.lastStalled,
            stallDuration / 1000,
          ];
          jsonObj.I23.stalling.push(stallObj);
        }
      }
      lastQoE = lolpQoE;
      // const boxBytes = events.load[events.load.length - 1].boxSize;
      if (downloadBytes === undefined) {
        sn -= 1;
      } else {
        PT = video.currentTime;
        //console.log("UTCtime: "+UTCtime, "PlaybackTime" + PT, "availibleB: "+availibleBW,"bitrate: "+bitrate.bitrate/1000, "currentBuffer: "+currentBuffer, "downloadTime: " + downloadTimeInMilliseconds/1000, 'downloadBytes: ' + downloadBytes);
        //if 	httpRequest.mediatype
        SavePlayerStats[SavePlayerStatsIndex] =
          UTCtime +
          ',' +
          PT +
          ',' +
          availibleBW.toFixed(0) +
          ',' +
          bitrate.toFixed(0) +
          ',' +
          currentLatency.toFixed(3) +
          ',' +
          (currentBuffer / 1000).toFixed(3) +
          ',' +
          stallNumber +
          ',' +
          lastStall +
          ...
          ',' +
          (downloadTimeInMilliseconds / 1000).toFixed(5) +
          ',' +
          downloadBytes +
          ',' +
          // boxBytes +
        ...
        lastStall = 0;
        console.log(
          'SavePlayerStatsIndex: ' + SavePlayerStatsIndex,
          'UTCtime: ' + UTCtime,
          'PlaybackTime' + PT,
          'availibleB: ' + availibleBW,
          'bitrate: ' + bitrate.bitrate / 1000,
          'currentBuffer: ' + currentBuffer,
          'downloadTime: ' + downloadTimeInMilliseconds / 1000,
          'downloadBytes: ' + downloadBytes
        );
        SavePlayerStatsIndex++;
        const distance = new Date().getTime() - startTime;
        if (
          SavePlayerStatsIndex == 1000000000 ||
          (Math.floor(distance / (1000 * 60)) == expTime && unfinished)
        ) {
          //console.log('HEEER');
          //StorePlayerStatsInFile(SavePlayerStats);
          var blobVideo = new Blob(SavePlayerStats, {
            type: 'raw',
            encoding: 'raw',
          });
          var FileTime = new Date().getTime();
          saveAs(blobVideo, FileTime + '-' + 'PlayerStats' + '.csv');
          unfinished = false;
          avgStats[1] =
            getMean(throughputArr) +
            ',' +
            getStandardDeviation(throughputArr) +
            ',' +
            Math.min(...throughputArr) +
            ',' +
            Math.max(...throughputArr) +
            ',' +
            getMean(bitrateArr) +
            ',' +
            getStandardDeviation(bitrateArr) +
            ',' +
            Math.min(...bitrateArr) +
            ',' +
            Math.max(...bitrateArr) +
            ...
            getMean(latencyArr) +
            ',' +
            getStandardDeviation(latencyArr) +
            ',' +
            Math.min(...latencyArr) +
            ',' +
            Math.max(...latencyArr) +
            ...
            getMean(bufferArr) +
            ',' +
            getStandardDeviation(bufferArr) +
            ',' +
            Math.min(...bufferArr) +
            ',' +
            Math.max(...bufferArr) +
            ...
            stallNumber +
            ',' +
            getSum(stallDurationArr) +
            ',' +
            ((getSum(stallDurationArr) / (expTime * 60)) * 100).toFixed(3) +
            ',' +
            _switchNumber +
            ',' +
            getMean(playbackRateArr) +
            ',' +
            getStandardDeviation(playbackRateArr) +
            ',' +
            Math.min(...playbackRateArr) +
            ',' +
            Math.max(...playbackRateArr) +
            ',' +
            getMean(QoEArr) +
            ',' +
            Math.min(...QoEArr) +
            ...
            ',' +
            '\r\n';
          var blobVideo = new Blob(avgStats, {
            type: 'raw',
            encoding: 'raw',
          });
          saveAs(blobVideo, FileTime + '-' + 'AvgStats' + '.csv');

          const json = JSON.stringify(jsonObj, null, 4);
          const blob = new Blob([json], { type: 'application/json' });
          saveAs(blob, 'mode0' + '-' + FileTime + '.json');

          const QoEFeeds = {
            bitrates: bitrateArr,
            stallDurAvg: getMean(stallDurationArr),
            stallDurSum: getSum(stallDurationArr),
            stallNumber: stallNumber,
            stallFreq: stallNumber / videoLen,
            startDelay: startDelay,
          };
          const QoEJson = JSON.stringify(QoEFeeds, null, 4);
          const QoEBlob = new Blob([QoEJson], { type: 'application/json' });
          saveAs(QoEBlob, 'QoEfeeds' + '-' + FileTime + '.json');
        }
      }
    }
  }, 100);
